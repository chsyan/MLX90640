...
class MLX90640:
    ...
    # default COM5, 9600 baud, I2C address 0x33
    def __init__(self, port='COM5', baud=9600, address=0x33, framerate=3, pattern=1):
        self.addr = address
        self.bus = serial.Serial(
            port, baud, timeout=4, write_timeout=4)  # open serial port
        ...
        self.CT1 = 40
        self.CT2 = 0
        # Restore sensitivity corrections
    ...
    def getRegs(self, reg, num):  # Reading multiple registers
        ...
        count = int(len(s)/2)
        # Unpack bytes as big-endian, signed integers
        integers = struct.unpack('>'+'H'*count, s)
        return integers
    ...
    def getPixDataRAM(self, i, j):
        ...
        pixOffset = Offsetavg + OCCRow * \
            pow(2, OCCscaleRow) + OCCCol*pow(2, OCCscaleCol) + \
            OffsetPix*pow(2, OCCscaleRem)
    ...
    def getCompensatedPixDataRAM(self, i, j):
        ...
        VIRcomp = VIREmcomp - self.TGC * \
            ((1-pattern)*pixOSCPSP0 + pattern*pixOSCPSP1)
        ...
        alpha = (alphaRef+(ACCRow << ACCScaleRow)+(ACCCol << ACCScaleCol) +
                 (alphaPixel << ACCScaleRem))/float(pow(2, alphaScale))

        alphaCPSP0 = (reg2439val & 0x03ff)/float(pow(2, alphaScaleCP))
        alphaCPSP1 = alphaCPSP0*(1+CPP1P0ratio/128.0)
        alphacomp = alpha - self.TGC * \
            ((1-pattern)*alphaCPSP0 + pattern*alphaCPSP1) * \
            (1+self.KsTa*(self.Ta-self.Ta0))
        # print(alphacomp)
        Tak4 = pow(self.Ta + 273.15, 4)
        Trk4 = pow(self.Ta-8 + 273.15, 4)
        Tar = Trk4-(Trk4-Tak4)/self.emissivity
        Sx = self.KsTo2*self.root4(pow(alphacomp, 3)
                                   * VIRcomp + pow(alphacomp, 4)*Tar)
